...
const App: React.FC = () => {
  ...
  const [copiedAllId, setCopiedAllId] = useState<boolean>(false);
...
useEffect(() => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (name === 'scriptCount' && parseInt(value, 10) > 5) {
        setShowPaywall(true);
        return;
    }
  ...
  const copyHistoryItem = (item: GeneratedScript) => {
    ...
    const fullContent = `--- SCRIPT ---\n${String(item.scriptContent || '')}${groundingChunksText}\n\n--- CAPTION ---\n${item.caption || ''}\n\n--- HASHTAGS ---\n${item.hashtags ? item.hashtags.map(t => t.startsWith('#') ? t : `#${t}`).join(' ') : ''}${longFormText}${imagePromptsText}`.trim();
  ...
  const copyAllGeneratedScripts = () => {
      const allText = generatedScripts.map((item, index) => {
          const imagePromptsText = item.imagePrompts && item.imagePrompts.length > 0 ? `\n\n--- VISUALISASI SCENE (PROMPTS) ---\n${item.imagePrompts.map(p => { let promptBlock = `Sumber: "${p.source}"\nPrompt Gambar: ${p.manualPrompt || p.prompt}`; if (p.textVideoPrompt) { promptBlock += `\nPrompt Text To Video: "${p.textVideoPrompt}"`; } if (p.videoPrompt) { promptBlock += `\nPrompt Image To Video: "${p.videoPrompt}"`; } if (p.veo3VideoPrompt) { promptBlock += `\nPrompt Image to Video (Veo 3): "${p.veo3VideoPrompt}"`; } return promptBlock; }).join('\n\n')}` : '';
          const groundingChunksText = item.groundingChunks && item.groundingChunks.length > 0 ? `\n\n--- SUMBER DARI GOOGLE SEARCH ---\n${item.groundingChunks.map(c => `${c.web.title || 'Untitled'}: ${c.web.uri}`).join('\n')}` : '';
          const longFormText = item.longFormContent ? `\n\n--- ARTIKEL / FACEBOOK POST ---\n${item.longFormContent}` : '';
          const fullContent = `--- SCRIPT ${index + 1} (Hook #${item.hookNumber}) ---\n${String(item.scriptContent || '')}${groundingChunksText}\n\n--- CAPTION ---\n${item.caption || ''}\n\n--- HASHTAGS ---\n${item.hashtags ? item.hashtags.map(t => t.startsWith('#') ? t : `#${t}`).join(' ') : ''}${longFormText}${imagePromptsText}`;
  ...
  const downloadAllGeneratedScripts = useCallback(() => {
      const allText = generatedScripts.map((item, index) => {
          const imagePromptsText = item.imagePrompts && item.imagePrompts.length > 0 ? `\n\n[VISUALISASI SCENE (PROMPTS)]\n${item.imagePrompts.map(p => { let promptBlock = `Sumber: "${p.source}"\nPrompt Gambar: ${p.manualPrompt || p.prompt}`; if (p.textVideoPrompt) { promptBlock += `\nPrompt Text To Video: "${p.textVideoPrompt}"`; } if (p.videoPrompt) { promptBlock += `\nPrompt Image To Video: "${p.videoPrompt}"`; } if (p.veo3VideoPrompt) { promptBlock += `\nPrompt Image to Video (Veo 3): "${p.veo3VideoPrompt}"`; } return promptBlock; }).join('\n\n')}` : '';
          const groundingChunksText = item.groundingChunks && item.groundingChunks.length > 0 ? `\n\n[SUMBER DARI GOOGLE SEARCH]\n${item.groundingChunks.map(c => `${c.web.title || 'Untitled'}: ${c.web.uri}`).join('\n')}` : '';
          const longFormText = item.longFormContent ? `\n\n[ARTIKEL / FACEBOOK POST]\n${item.longFormContent}` : '';
      ...
      const filename = `semua_skrip_generasi_${new Date().getTime()}.txt`;
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center p-4 sm:p-6 lg:p-8">
      ...
      <header className="text-center mb-6 w-full max-w-7xl relative">
        <button 
            onClick={() => setShowManualBook(true)}
            className="absolute top-0 right-0 z-40 bg-slate-800 border border-slate-700 text-amber-500 hover:text-amber-400 hover:bg-slate-700 px-3 py-2 rounded-lg text-sm font-bold flex items-center gap-2 shadow-lg transition-all"
        >
        ...
        <div className="mt-4 flex flex-col items-center justify-center max-w-md mx-auto">
          ...
          <p className="text-xs text-slate-500 text-center">
            Jika Kuota Server Habis masukan APIkey Anda sendiri secara manual di atas, API Key hanya tersimpan di browser Anda. Tutorial Cara mendapatkan apikey sendiri silakan buka di 
            <a href="https://www.akariu.my.id/p/buat-apikey-google-gratis.html" target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:underline mx-1">
              sini
            </a>. 
            Paste API key anda 
                ...
                <fieldset className="space-y-6 animate-fade-in">
                    ...
                    <fieldset disabled={!isAiScriptMode} className="space-y-6 disabled:opacity-50">
                        ...
                        <div className="flex items-end gap-4">
                            ...
                            <div className="w-32 flex-shrink-0">
                                ...
                                <input id="scriptCount" name="scriptCount" type="number" value={formData.scriptCount} onChange={handleInputChange} min="1" max="100" className="w-full bg-slate-700 border border-slate-600 rounded-md px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:cursor-not-allowed" />
                            </div>
                        </div>
                    </fieldset>
                    ...
                    <div className="space-y-4 pt-4 border-t border-slate-700/50">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <input type="checkbox" id="generateImagePrompts" name="generateImagePrompts" checked={formData.generateImagePrompts} onChange={e => { e.preventDefault(); setShowPaywall(true); }} className="sr-only peer" />
                            <div className="relative w-11 h-6 bg-slate-600 rounded-full peer peer-focus:ring-2 peer-focus:ring-indigo-500 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600 cursor-pointer" onClick={(e) => { e.preventDefault(); setShowPaywall(true); }}></div>
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <input type="checkbox" id="enableLipsync" name="enableLipsync" checked={formData.enableLipsync} onChange={e => { e.preventDefault(); setShowPaywall(true); }} className="sr-only peer" />
                            <div className="relative w-11 h-6 bg-slate-600 rounded-full peer peer-focus:ring-2 peer-focus:ring-indigo-500 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600 cursor-pointer" onClick={(e) => { e.preventDefault(); setShowPaywall(true); }}></div>
                        </div>
                        ...
                        {formData.mode === Mode.SALES && (
                            <fieldset disabled={!isAiScriptMode} className="disabled:opacity-50 border-t border-slate-700/50 pt-4 mt-4">
                                <div>
                                    <label className="block text-lg font-bold text-red-500 mb-2">Mode Produksi Konten Jualan</label>
                                    <div className="bg-slate-700/50 p-3 rounded-lg text-xs text-slate-300 mb-3 border border-slate-600">
                                        <strong>Spesialis:</strong> Hanya pakai 1 rumus Bahas promo (biasanya untuk Shopee Video).<br/>
                                        <strong>Kompleks:</strong> Menggunakan banyak full rumus struktur konten automatis.
                                    </div>
                                    <div className="grid grid-cols-2 gap-2 bg-slate-700 p-1 rounded-lg">
                                        <button type="button" onClick={() => setFormData(p => ({ ...p, salesMode: 'spesialis' }))} className={`px-4 py-2 rounded-md text-sm font-semibold transition-colors duration-200 ${formData.salesMode === 'spesialis' ? 'bg-gradient-to-r from-yellow-400 to-yellow-600 text-white shadow' : 'text-slate-300 hover:bg-slate-600/50'}`}>Spesialis</button>
                                ...
                                {formData.salesMode === 'spesialis' && (
                                    <div className="mt-4 space-y-4 animate-fade-in">
                                        <div>
                                            ...
                                            <div className="grid grid-cols-2 gap-2 bg-slate-700 p-1 rounded-lg">
                                                <button type="button" onClick={() => setFormData(p => ({ ...p, spesialisType: 'aman' }))} className={`px-4 py-2 rounded-md text-sm font-semibold transition-colors duration-200 ${formData.spesialisType === 'aman' ? 'bg-gradient-to-r from-yellow-400 to-yellow-600 text-white shadow' : 'text-slate-300 hover:bg-slate-600/50'}`}>Aman</button>
                                                <button type="button" onClick={() => setFormData(p => ({ ...p, spesialisType: 'bebas' }))} className={`px-4 py-2 rounded-md text-sm font-semibold transition-colors duration-200 ${formData.spesialisType === 'bebas' ? 'bg-gradient-to-r from-yellow-400 to-yellow-600 text-white shadow' : 'text-slate-300 hover:bg-slate-600/50'}`}>Bebas</button>
                                            </div>
                                        </div>
                                        <div>
                    ...
                    {(formData.mode !== Mode.SALES || formData.salesMode !== 'spesialis') && (
                        <fieldset disabled={!isAiScriptMode} className="disabled:opacity-50 mt-4">
                            <div>
                                ...
                                <div>
                                    <label htmlFor="ctaType" className="block text-lg font-bold text-red-500 mb-1">Jenis CTA (Call to Action)</label>
                                ...
                                </div></div>) : (<div className="max-h-40 overflow-y-auto space-y-2 pr-2">{STORYTELLING_FORMULAS.map(formula => (<button type="button" key={formula.id} onClick={() => handleFormulaToggle(formula.id)} className={`w-full text-left text-xs px-3 py-2 rounded-md transition-colors duration-200 flex items-center justify-between ${formData.selectedFormulas.includes(formula.id) ? 'bg-indigo-500/30 text-indigo-300 ring-1 ring-indigo-500' : 'bg-slate-700/50 text-slate-300 hover:bg-slate-600/50'}`}><span>{formula.label}</span>{formData.selectedFormulas.includes(formula.id) && <Icon type="check" className="w-4 h-4" />}</button>))}</div>)}
                            </div>
                        </fieldset>
                    )}
                    ...
                    {generatedScripts.length > 0 && (
                        <div className="space-y-6" ref={resultsRef}>
                             <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                                ...
                                <div className="flex gap-2">
                                    <button 
                                        onClick={downloadAllGeneratedScripts}
                                        className="flex flex-1 sm:flex-none items-center justify-center gap-2 px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg transition text-sm font-semibold"
                                        title="Download semua skrip"
                                    >
                                        <Icon type="download" className="w-4 h-4" />
                                        Download All
                                    </button>
                                    <button 
                                        onClick={copyAllGeneratedScripts}
                                        className="flex flex-1 sm:flex-none items-center justify-center gap-2 px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg transition text-sm font-semibold"
                                        title="Copy semua skrip"
                                    >
                                        {copiedAllId ? <Icon type="check" className="w-4 h-4 text-green-500" /> : <Icon type="copy" className="w-4 h-4" />}
                                        Copy All
                                    </button>
                                </div>
                            </div>
        ...
        >
            ...
            >
                ...
                <div className="text-center mb-6">
                    ...
                    <h3 className="text-xl font-bold text-white mb-2">Dapatkan Full Version</h3>
                    <p className="text-slate-300 text-sm">
